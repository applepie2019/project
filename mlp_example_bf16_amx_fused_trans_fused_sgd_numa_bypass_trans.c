                                 ...
                                 libxsmm_blasint bc, libxsmm_blasint bk, libxsmm_blasint threads, my_eltwise_fuse fuse_type, float lr)
{
  ...
  if (threads == 16) {
    ...
    res.bwd_col_teams = 2;
    res.bwd_row_teams = 1;
    res.bwd_M_hyperpartitions = 20;
    res.bwd_N_hyperpartitions = 1;
    res.upd_bf = 1;
    res.upd_2d_blocking = 1;
    res.upd_col_teams = 2;
    res.upd_row_teams = 1;
    res.upd_M_hyperpartitions = 20;
    res.upd_N_hyperpartitions = 1;
    res.ifm_subtasks = 1;
    res.ofm_subtasks = 1;
  } else {
  ...
  res.gemm_bwd = libxsmm_bsmmdispatch_reducebatch_strd_unroll(res.bc, res.bn, res.bk, res.bk*res.C*sizeof(libxsmm_bfloat16), res.bk*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &ldb, &lda, &ldb, &alpha, &beta, &l_flags, NULL);
  if ( res.gemm_bwd == NULL ) {
    fprintf( stderr, "JIT for BRGEMM TPP gemm_bwd failed. Bailing...!\n");
    exit(-1);
  }
  res.gemm_bwd2 = libxsmm_bsmmdispatch_reducebatch_strd_unroll(res.bc, res.bn, res.bk, res.bk*res.C*sizeof(libxsmm_bfloat16), res.bk*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &ldb, &lda, &ldb, &alpha, &zerobeta, &l_flags, NULL);
  if ( res.gemm_bwd2 == NULL ) {
    fprintf( stderr, "JIT for BRGEMM TPP gemm_bwd2 failed. Bailing...!\n");
    exit(-1);
  }
  res.gemm_bwd3 = libxsmm_bmmdispatch_reducebatch_strd_unroll(res.bc, res.bn, res.bk, res.bk*res.C*sizeof(libxsmm_bfloat16), res.bk*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &ldb, &lda, &ldb, &alpha, &zerobeta, &l_flags, NULL);
  ...
  res.gemm_bwd5 = libxsmm_bmmdispatch_reducebatch_strd_meltwfused_unroll(res.bc, res.bn, res.bk, res.bk*res.C*sizeof(libxsmm_bfloat16), res.bk*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &ldb, &lda, &ldb, &alpha, &zerobeta, &l_flags, NULL, bwd_fused_op, LIBXSMM_DATATYPE_BF16, fusion_flags_bwd, 0, 0, 0, 0);
  ...
  res.gemm_upd = libxsmm_bsmmdispatch_reducebatch_strd_unroll(updM, updN, res.bn, res.K*res.bn*sizeof(libxsmm_bfloat16), res.C*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &lda, &ldb, &ldc, &alpha, &beta, &l_flags, NULL);
  if ( res.gemm_upd == NULL ) {
    fprintf( stderr, "JIT for BRGEMM TPP gemm_upd failed. Bailing...!\n");
    exit(-1);
  }
  res.gemm_upd2 = libxsmm_bsmmdispatch_reducebatch_strd_unroll(updM, updN, res.bn, res.K*res.bn*sizeof(libxsmm_bfloat16), res.C*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &lda, &ldb, &ldc, &alpha, &zerobeta, &l_flags, NULL);
  if ( res.gemm_upd2 == NULL ) {
    fprintf( stderr, "JIT for BRGEMM TPP gemm_upd2 failed. Bailing...!\n");
    exit(-1);
  }
  l_flags = l_flags | LIBXSMM_GEMM_FLAG_VNNI_C;
  res.gemm_upd3 = libxsmm_bmmdispatch_reducebatch_strd_unroll(updM, updN, res.bn, res.K*res.bn*sizeof(libxsmm_bfloat16), res.C*res.bn*sizeof(libxsmm_bfloat16), unroll_hint, &lda, &ldb, &ldc, &alpha, &zerobeta, &l_flags, NULL);
...
void my_fc_fwd_exec( my_fc_fwd_config cfg, const libxsmm_bfloat16* wt_ptr, const libxsmm_bfloat16* in_act_ptr, libxsmm_bfloat16* out_act_ptr,
                     const libxsmm_bfloat16* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch, int block_a, int block_b )
{
  ...
  if (use_2d_blocking == 1) {
    ...
    col_teams    = cfg.fwd_col_teams; //1 //2
    row_teams = cfg.fwd_row_teams; //20 //1
    hyperteam_id = ltid/(col_teams*row_teams);
    _nBlocksOFm  = nBlocksOFm/cfg.fwd_M_hyperpartitions;  //1 //20
    _nBlocksMB   = nBlocksMB/cfg.fwd_N_hyperpartitions;   //2  //1
  ...
  if (use_2d_blocking == 1) {
    if (BF > 1) {
      ...
      for (ofm1 = my_M_start; ofm1 < my_M_end; ofm1+=block_a) {
        for (mb1 = my_N_start; mb1 < my_N_end; mb1+=block_b) {
	  for (int i_a = ofm1; i_a < (ofm1 + block_a); ++i_a) {
	    for (int i_b = mb1; i_b < (mb1 + block_b); ++i_b) {
              if (((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS)||((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU)) {
                if ((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS) {
                  gemm_eltwise_params.bias_ptr  = (float*) fp32_bias_scratch + i_a * cfg.bk;
                }
                if ((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU) {
                  gemm_eltwise_params.out_ptr   = &LIBXSMM_VLA_ACCESS(4, relubitmask, i_b, i_a, 0, 0, nBlocksOFm, cfg.bn, cfg.bk/32);
                }
                bf16_batchreduce_kernel_zerobeta_fused_eltwise( &LIBXSMM_VLA_ACCESS(5, filter, i_a, 0, 0, 0, 0, nBlocksIFm, bc_lp, cfg.bk, lpb),
                  &LIBXSMM_VLA_ACCESS(4, input,  i_b, 0,  0, 0, nBlocksIFm, cfg.bn, cfg.bc),
                  &LIBXSMM_VLA_ACCESS(4, output, i_b,  i_a, 0, 0, nBlocksOFm, bn, bk), &blocks, &gemm_eltwise_params);
             } else {
               cfg.gemm_fwd3( &LIBXSMM_VLA_ACCESS(5, filter, i_a, 0, 0, 0, 0, nBlocksIFm, bc_lp, cfg.bk, lpb),
                 &LIBXSMM_VLA_ACCESS(4, input,  i_b, 0,  0, 0, nBlocksIFm, cfg.bn, cfg.bc),
                 &LIBXSMM_VLA_ACCESS(4, output, i_b,  i_a, 0, 0, nBlocksOFm, bn, bk), &blocks);
              }
            }
          }
        }
...
void my_fc_bwd_exec( my_fc_bwd_config cfg,  libxsmm_bfloat16* wt_ptr, libxsmm_bfloat16* din_act_ptr,
                     ...
                     libxsmm_bfloat16* dbias_ptr, const unsigned char* relu_ptr, my_pass pass, int start_tid, int my_tid, void* scratch, float *fil_master, int block_a, int block_b )
{
...
if (cfg.upd_2d_blocking == 0) {
  ...
  if (cfg.bwd_2d_blocking == 0) {
    ...
    if (use_2d_blocking == 1) {
      if (BF > 1) {
        for ( ofm1 = 0; ofm1 < BF; ++ofm1 ) {
          for (ifm1 = my_M_start; ifm1 < my_M_end; ++ifm1) {
            for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
              ...
              cfg.gemm_bwd( &LIBXSMM_VLA_ACCESS(5, filter,   ofm1*KB_BLOCKS, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb),
        ...
        for (ifm1 = my_M_start; ifm1 < my_M_end; ifm1+=block_a) {
          for (mb1 = my_N_start; mb1 < my_N_end; mb1+=block_b) {
	    for (int i_a = ifm1; i_a < (ifm1 + block_a); ++i_a) {
	       for (int i_b = mb1; i_b < (mb1 + block_b); ++i_b) {
		 if (cfg.fuse_relu_bwd > 0) {
		   eltwise_params_bwd.relu_bitmask_bwd = &LIBXSMM_VLA_ACCESS(4, relubitmask, i_b, i_a, 0, 0, nBlocksIFm, cfg.bn, cfg.bc/32);
		   cfg.gemm_bwd5( &LIBXSMM_VLA_ACCESS(5, filter,  0, i_a, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb),
		       &LIBXSMM_VLA_ACCESS(4, doutput,   i_b,  0, 0, 0, nBlocksOFm, bn, bk),
		       &LIBXSMM_VLA_ACCESS(4, dinput,    i_b,  i_a, 0, 0, nBlocksIFm, bn, bc), &blocks, &eltwise_params_bwd);
		 } else {
		   cfg.gemm_bwd3( &LIBXSMM_VLA_ACCESS(5, filter,  0, i_a, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb),
		       &LIBXSMM_VLA_ACCESS(4, doutput,   i_b,  0, 0, 0, nBlocksOFm, bn, bk),
		       &LIBXSMM_VLA_ACCESS(4, dinput,    i_b,  i_a, 0, 0, nBlocksIFm, bn, bc), &blocks);
		 }
	      }
	    }
          }
  ...
  if (use_2d_blocking == 1) {
    ...
    col_teams    = cfg.upd_col_teams; //1
    row_teams = cfg.upd_row_teams;  //20
    hyperteam_id = ltid/(col_teams*row_teams);
    _nBlocksOFm  = nBlocksOFm/cfg.upd_M_hyperpartitions; //1
    _nBlocksIFm  = nBlocksIFm/cfg.upd_N_hyperpartitions;  //2
    _ltid = ltid % (col_teams * row_teams);
  ...
  if (cfg.upd_2d_blocking == 0) {
    ...
    if (use_2d_blocking == 1) {
      ...
      if (BF == 1) {
        for (ofm1 = my_M_start; ofm1 < my_M_end; ofm1+=block_a) {
          for (ifm1 = my_N_start; ifm1 < my_N_end; ifm1+=block_b) {
	    for (int i_a = ofm1; i_a < (ofm1 + block_a); ++i_a) {
	       for (int i_b = ifm1; i_b < (ifm1 + block_b); ++i_b) {
            /* Transpose input block */
		 cfg.gemm_upd3(&LIBXSMM_VLA_ACCESS(4, doutput, 0, i_a, 0, 0, nBlocksOFm, bn, bk), &LIBXSMM_VLA_ACCESS(4, input, 0, i_b, 0, 0, nBlocksIFm, bn, bc), &LIBXSMM_VLA_ACCESS(5, dfilter, i_a, i_b, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb), &blocks);
#ifndef BYPASS_SGD
		 {
		 __m512 vlr = _mm512_set1_ps( cfg.lr );
		 libxsmm_bfloat16 *dwt_bf16 = (libxsmm_bfloat16*)  &LIBXSMM_VLA_ACCESS(5, dfilter, i_a, i_b, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb);
		 libxsmm_bfloat16 *wt_bf16  = (libxsmm_bfloat16*)  &LIBXSMM_VLA_ACCESS(5, filter,  i_a, i_b, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb);
		 float *wt_fp32  = (float*)             &LIBXSMM_VLA_ACCESS(4, master_filter,  i_a, i_b, 0, 0, nBlocksIFm, bc, bk);
        ...
        for (bfn = 0; bfn < BF; bfn++) {
          for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
            for (ifm1 = my_N_start; ifm1 < my_N_end; ++ifm1) {
              ...
              cfg.gemm_upd(&LIBXSMM_VLA_ACCESS(4, doutput, bfn*blocks, ofm1, 0, 0, nBlocksOFm, bn, bk), &LIBXSMM_VLA_ACCESS(4, input, bfn*blocks, ifm1, 0, 0, nBlocksIFm, bn, bc), &LIBXSMM_VLA_ACCESS(4, dfilter_f32, ofm1, ifm1, ifm2*bbc, ofm2*bbk, nBlocksIFm, bc, bk), &blocks);
...
int main(int argc, char* argv[])
{
  ...
  if (argc > i) a_block    = atoi(argv[i++]);
  if (argc > i) b_block    = atoi(argv[i++]);
  /* allocate the number of channles buffer */
  if ( num_layers < 1 ) {
    printf("Usage: %s iters MB fuse_type type bn bk bc block_a block_b C1 C2 ... CN\n", argv[0]);
    return 0;
  }
...
#endif
      for (j = 0; j < iters; ++j) {
        if ( tid == 0 ) { SimMarker(1, j); }
        for ( i = 0; i < num_layers; ++i) {
          my_fc_fwd_exec( my_fc_fwd[i], fil_libxsmm[i], act_libxsmm[i], act_libxsmm[i+1],
                          bias_libxsmm[i], relumask_libxsmm[i], 0, tid, scratch, a_block, b_block);
        }
        if ( tid == 0 ) { SimMarker(2, j); }
      }
    }
      ...
      for (j = 0; j < iters; ++j) {
        ...
        for ( i = num_layers-1; i > 0; --i) {
          my_fc_bwd_exec( my_fc_bwd[i], fil_libxsmm[i], delact_libxsmm[i], delact_libxsmm[i+1], delfil_libxsmm[i],
                          act_libxsmm[i], delbias_libxsmm[i], relumask_libxsmm[i], MY_PASS_BWD, 0, tid, scratch, fil_master[i], a_block, b_block );
        }
        my_fc_bwd_exec( my_fc_bwd[0], fil_libxsmm[0], delact_libxsmm[0], delact_libxsmm[0+1], delfil_libxsmm[0],
                        act_libxsmm[0], delbias_libxsmm[0], relumask_libxsmm[0], MY_PASS_BWD_W, 0, tid, scratch, fil_master[0], a_block, b_block );
        if ( tid == 0 ) { SimMarker(2, j); }
      }
    }
    l_end = libxsmm_timer_tick();
...
#   pragma omp parallel private(i,j)
#endif
    {
      ...
      for (j = 0; j < iters; ++j) {
        ...
        for ( i = num_layers-1; i > 0; --i) {
          my_fc_bwd_exec( my_fc_bwd[i], fil_libxsmm[i], delact_libxsmm[i], delact_libxsmm[i+1], delfil_libxsmm[i],
                          act_libxsmm[i], delbias_libxsmm[i], (my_fc_bwd[i].fuse_relu_bwd > 0) ? relumask_libxsmm[i-1] : relumask_libxsmm[i], MY_PASS_BWD, 0, tid, scratch, fil_master[i], a_block, b_block );
        }
        my_fc_bwd_exec( my_fc_bwd[0], fil_libxsmm[0], delact_libxsmm[0], delact_libxsmm[0+1], delfil_libxsmm[0],
                        act_libxsmm[0], delbias_libxsmm[0], relumask_libxsmm[0], MY_PASS_BWD_W, 0, tid, scratch, fil_master[0], a_block, b_block );
        if ( tid == 0 ) { SimMarker(2, j); }
